...
#Run Mysql container
sudo docker run --name mysql -e MYSQL_ROOT_PASSWORD=coinBase$ -d zhaomingli/mysql8:coinbase
#Run data pipeline container
docker run --name pipeline --link mysql:coinbase-mysql -t pyscala:coinbase
